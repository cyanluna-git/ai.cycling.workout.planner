...
이 서비스는 AI가 워크아웃을 처음부터 창작하지 않고, **검증된 워크아웃 모듈 메뉴판**에서 최적의 코스를 선택하는 방식을 사용합니다.

1. **메뉴판 (Library)**: 
   - 검증된 워밍업, 메인 본운동(VO2max, SST, 역치 등), 쿨다운 모듈이 미리 정의되어 있습니다.
   - 예: `sst_10min` (스윗스팟 10분), `vo2_3x3` (VO2max 3분x3회)

2. **지능적 선택 (AI Selector)**:
   - 사용자의 **TSB(컨디션)**와 **목표 시간**을 분석합니다.
   - 메뉴판에서 상황에 딱 맞는 모듈을 순서대로 골라 담습니다.
   - 예: "피곤하니까(TSB -20) 강도는 낮게, 시간은 60분을 채워야지 -> 웜업 + 템포 + 쿨다운"

3. **정확한 조립 (Assembler)**:
   - AI가 주문한 순서대로 블록을 조립하여 최종 워크아웃 파일을 생성합니다.
   - 이를 통해 **검증된 훈련 퀄리티**와 **AI의 유연한 구성**을 동시에 달성합니다.
