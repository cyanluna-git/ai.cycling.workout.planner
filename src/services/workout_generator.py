...
class CoachingNote(BaseModel):
    """Structured coaching guidance for workout selection."""
...
class GeneratedWorkout:
    ...
    coaching: Optional[CoachingNote] = None  # Structured coaching note
        ...
        try:
            ...
            if profile_selection and "profile_id" in profile_selection:
                ...
                if profile:
                    ...
                    coaching_data = profile_selection.get("coaching")
                    coaching = None
                    if coaching_data and isinstance(coaching_data, dict):
                        try:
                            coaching = CoachingNote(**coaching_data)
                        except Exception as e:
                            logger.warning(f"Failed to parse coaching note: {e}")
                    
                    # Backward compatibility: use design_goal if coaching not provided
                    design_goal_text = None
                    if coaching:
                        design_goal_text = coaching.selection_reason
                    elif profile_selection.get("design_goal"):
                        design_goal_text = profile_selection.get("design_goal")
