...
- Power: FTP Percent (50%, 88%, 100%, 115%)

Step Examples:
- "10m 50%" = 10 minutes at 50% FTP
- "5m 100%" = 5 minutes at 100% FTP
...
- TSB -20 ~ -10 (Fatigued): Endurance (65-75%, under 60 mins)
- TSB -10 ~ 0 (Neutral): Tempo/Sweet Spot allowed
- TSB > 0 (Fresh): Threshold/VO2max allowed

{syntax_guide}

**Output Rules (Strictly Follow):**
Respond strictly in the JSON format below. Do not add any extra explanation or markdown. Output raw JSON only.

```json
{{
  "name": "Workout Name (English)",
...
Based on the information above, generate a suitable cycling workout for today.
"""


# Training style descriptions for the prompt
STYLE_DESCRIPTIONS = {
    "auto": "Auto-decide based on TSB state",
    "polarized": "Polarized - 80% Easy (Z1-Z2) + 20% Very Hard (Z5-Z6), avoid middle intensity",
    "norwegian": "Norwegian - 4x8m or 5x5m Z4 (Threshold) intervals",
    "pyramidal": "Pyramidal - Based on Z1-Z2, adding Z3-Z4, minimize Z5",
    "threshold": "Threshold - Intervals near FTP (95-105%)",
    "sweetspot": "Sweet Spot - Long intervals at 88-94% FTP",
    "endurance": "Endurance - Z2 focused long ride",
}

INTENSITY_DESCRIPTIONS = {
    "auto": "Auto-decide based on TSB state",
    "easy": "Recovery/Easy (Z1-Z2 only)",
...
class WorkoutGenerator:
    ...
    def _build_user_prompt(
        ...
        environment = (
            "Indoor Trainer (Structured intervals, short rest)" if indoor else "Outdoor or General"
        )

        # Format user notes
        user_notes = f"- Additional Notes: {notes}" if notes else ""
